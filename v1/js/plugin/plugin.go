...
type jsPlugin struct{ v js.Value }
...
func (p jsPlugin) Install(a *core.App) {
	if fn := p.v.Get("router"); fn.Type() == js.TypeFunction {
		a.RegisterRouter(func(path string) { fn.Invoke(path) })
	}
	if fn := p.v.Get("store"); fn.Type() == js.TypeFunction {
		a.RegisterStore(func(module, store, key string, value any) {
			fn.Invoke(module, store, key, js.ValueOf(value))
		})
	}
	if fn := p.v.Get("template"); fn.Type() == js.TypeFunction {
		a.RegisterTemplate(func(id, html string) { fn.Invoke(id, html) })
	}
	mount := p.v.Get("mount")
	unmount := p.v.Get("unmount")
	if mount.Type() == js.TypeFunction || unmount.Type() == js.TypeFunction {
		a.RegisterLifecycle(
