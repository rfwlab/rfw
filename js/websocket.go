...
func (s *Socket) deliver(event Value, handlers SocketHandlers) {
	...
	if data.Type() == TypeString {
		// The payload has to be converted before it can be measured: Get and
		// Length both panic on a JavaScript string primitive, so there is no
		// way to read .length from Go without materialising the value. The
		// browser already holds the string, so the copy is bounded by what it
		// accepted, and the limit still keeps it out of the inbound queue.
		text := data.String()
